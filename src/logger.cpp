...
void cse4589_init_log(char* port)
{
    ...
    uint32_t bufferSize = HOSTNAME_LEN;

#ifdef __APPLE__
    // get executable path for mac
    if (_NSGetExecutablePath(exec_path, &bufferSize) != 0) {
        printf("Mac: Oops! Failed to get executable path. Contact the course staff!\n");
        exit(1);
    }
#else
